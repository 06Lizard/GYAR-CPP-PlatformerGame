...
void Player::Move() {
	...
	// 4. continue jump if w is still pressed and jumpTime is less than 3
		...
		if (((states >> 6) & 0b11) < 3 && !mapp->mapp[x][y - 1]->getCollision()) {
			y--; // Move up
	...
	if (states & Left && !(states & Right)) {
		if (x > 0 && !mapp->mapp[x - 1][y]->getCollision()) // colition check & border
			x--;
	}
	else if (!(states & Left) && states & Right) {
		if (x < mapp->width-1 && !mapp->mapp[x + 1][y]->getCollision()) // colition check & border
			x++;
	}
}
// need work
...
{
	if (mapp->mapp[x][y] && (dynamic_cast<Flag*>(mapp->mapp[x][y]) || dynamic_cast<FlagPole*>(mapp->mapp[x][y]))) // if player is on flag
	{
